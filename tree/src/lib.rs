...
impl<T,E> Zipper<T,E> {
    ...
    pub fn down(& mut self, i : usize) -> Result<(),&str> {
        ...
        self.frames.push(ZipperFrame {val : old_focus.val, edge, left, right});
        return Ok(());
    }
    pub fn up(& mut self) -> Result<(), &str> {
        self.frames.pop().map(|ZipperFrame{val, edge, mut left, right}| {
    ...
    pub fn left(& mut self) -> Result<(), &str> {
        ...
        self.frames.get_mut(0).and_then(|& mut ZipperFrame{ref mut left, ref mut right, ref mut edge,..}|
    ...
    pub fn push(& mut self, x : T, edge : E) {
        ...
        let old_focus = mem::replace(&mut self.focus, new_focus);
        self.frames.push(ZipperFrame {val : old_focus.val, edge, left, right:Vec::new()});
    }
}
